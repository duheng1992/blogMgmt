				...
				<el-form-item label="头像">
					<el-upload
					  ...
					  :before-upload="beforeAvatarUpload">
					   ...
					   <!-- <el-button style="margin-left: 10px;" size="small" type="success" @click.native="handleAvatarSuccess">上传到服务器</el-button> -->
	...
	export default {
		...
		methods: {
			...
			//console.log("----------")
				...
				this.loading = true;
				//NProgress.start();
				getAdminUserList(para).then((res) => {
			...
			handleEdit: function (index, row) {
				...
				this.imageServer = this.imageServer + '/' + this.editForm.user_id;  //用于跟用户关联(存储头像用)
			},
			//覆盖默认的上传行为
            httprequest() {
						...
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							modifyUser(para).then((res) => {
								...
								if(res.resCode == '01'){
									...
									//此时上传头像(有userid了),是一个单独的ajax，url是其action
									this.$refs.upload.submit();
									//console.log(r)
									this.$refs['editForm'].resetFields();
									//this.editFormVisible = false; //保存后页面不关闭
									this.getUser();
								} else {
									this.$message({
										message: '用户更新失败',
		 ...
		 //    },
		    beforeAvatarUpload(file) {
		        ...
		        if (!isJPG) {
		          this.$message.error('头像必须为JPG格式!');
		        }
		        if (!isLt2M) {
		          this.$message.error('头像应小于2MB!');
		        }
		        return isJPG && isLt2M;
		    },
		    onChangeAvatar(){
